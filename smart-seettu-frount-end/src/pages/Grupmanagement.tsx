...
// // import { myData } from "../layout/Dashbord";
...
//   const [userData, setUserData] = useState<ctype>({
//     id: "",
//     name: "",
//     email: "",
//     contact: "",
...
//   });
//   // ----------------------------
//   // Event Listener එකක් දාලා වෙනස් වීම් අහන් ඉන්නවා
//   window.addEventListener("myDataChanged", (e: Event) => {
//     const customEvent = e as CustomEvent;
//     // console.log("Variable එක වෙනස් වුණා! අලුත් අගය:", customEvent.detail);
//     // alert(`Variable එක වෙනස් වුණා! අලුත් අගය:${customEvent.detail}`);
//     const userObject = customEvent.detail;
...
//   const handleMemberInputChangeNextMemberId = (e: any) => {
//     setNewMember({ ...newMember, memberId: " " });
//   };

//   const handleMemberInputChangename = (e: any) => {
//     setNewMember({ ...newMember, membername: userData.name });
//   };

//   const handleMemberInputChangeContact = (e: any) => {
//     setNewMember({ ...newMember, contactnumber: userData.contact });
//   };

//   const addMemberToList = (e: any) => {
//     // alert(newMember.membername);
//     e.preventDefault();
//     if (
//       !newMember.memberId ||
...
//                     readOnly
//                     onChange={handleMemberInputChangeNextMemberId}
//                     placeholder="USR-102"
//                     className="w-full bg-gradient-to-r from-gray-50 to-white border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100 transition-all duration-200"
//                   />
//                 </div>
...
//                     value={userData.name}
//                     // readOnly
//                     onChange={handleMemberInputChangename}
...
//                     // readOnly
//                     onChange={handleMemberInputChangeContact}
...
import Swal from "sweetalert2";
import { createGrup, getNextGrupId } from "../service/grup";
import { useSelector } from "react-redux";
import { type RootState } from "../redux/store";
import { useNavigate } from "react-router";
...
const Grupmanagement = () => {
  ...
  // Redux store එකෙන් admin විස්තර ගැනීම
  const profailuser = useSelector(
    (state: RootState) => state.customer.currentCustomer,
  );
  ...
  // 1. Event Listener එක නිවැරදිව useEffect එකක් ඇතුළත හැසිරවීම
  useEffect(() => {
    const handleDataChange = (e: Event) => {
      ...
      // Event එකෙන් ලැබෙන දත්ත කෙලින්ම අලුත් සාමාජිකයාගේ fields වලට ඇතුලත් කිරීම
      setNewMember((prev) => ({
        ...prev,
        membername: userObject.name || "",
        contactnumber: userObject.contact || "",
      }));
    };

    window.addEventListener("myDataChanged", handleDataChange);

    // Component එක අයින් වන විට Listener එක අයින් කිරීම (Cleanup)
    return () => {
      window.removeEventListener("myDataChanged", handleDataChange);
    };
  }, []);

  // 2. සාමාජිකයින්ගේ ලැයිස්තුව වෙනස් වන විට ඊළඟ Member ID එක හැදීම
  useEffect(() => {
  ...
  // Inputs Type කරන විට state එක update වන ආකාරය නිවැරදි කිරීම
  ...
  const saveToServer = async () => {
    const admin: any = profailuser;
    if (!admin) {
      alert("Admin දත්ත සොයාගත නොහැක.");
      return;
    }

    // Admin ව වෙනම object එකක් ලෙස සකසා ලැයිස්තුවට එකතු කිරීම
    const adminObject = {
      memberId: `M-${String(members.length + 1).padStart(3, "0")}`,
      email: admin.email,
      membername: admin.name,
      contactnumber: admin.phone || admin.contact,
      tagname: "admin",
    };

    const newmembersList = [...members, adminObject];
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    value={newMember.memberId}
                    readOnly
                    className="w-full bg-gray-100 border border-gray-200 rounded-xl px-4 py-3 text-sm cursor-not-allowed focus:outline-none"
                  />
                </div>
                <div>
                  <label className="block text-xs text-cyan-950 mb-2 font-semibold uppercase tracking-wide">
